...
public class OldPerson extends User {
    ...
    public String writeRequest(RequestType requestType) throws WrongTypeExeption {
        ArrayList<RequestType> availableRequestTypes = getAvailableRequestTypes(new OldPerson());

        if (!availableRequestTypes.contains(requestType))
            throw new WrongTypeExeption("Utilizatorul de tip pensionar nu poate inainta o cerere de tip " + getRequestName(requestType));

        return "Subsemnatul " + this.getName() + ", va rog" + " sa-mi aprobati urmatoarea " + "solicitare: " + getRequestName(requestType);
    }
    ...
    public Request createRequest(RequestType requestType, Integer priority, LocalDateTime localDateTime, String fileName) throws IOException {
        ...
        try{
            ...
            bufferedWriter.write("Utilizatorul de tip pensionar nu poate inainta o cerere de tip " + getRequestName(requestType)+"\n");
