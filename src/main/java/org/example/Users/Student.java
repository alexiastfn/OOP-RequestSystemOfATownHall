...
public class Student extends User{
    ...
    public String writeRequest(RequestType requestType) throws WrongTypeExeption {
        ArrayList<RequestType> availableRequestTypes =  getAvailableRequestTypes(new Student());

        if (!availableRequestTypes.contains(requestType))
            throw new WrongTypeExeption("Utilizatorul de tip elev nu poate inainta o cerere de tip "+ getRequestName(requestType));

        return "Subsemnatul "+ this.getName() + ", elev la scoala " +this.school+", va rog " +
    ...
    public Request createRequest(RequestType requestType, Integer priority, LocalDateTime localDateTime,String fileName) throws IOException {
        ...
        try {
            ...
            bufferedWriter.write("Utilizatorul de tip elev nu poate inainta o cerere de tip " + getRequestName(requestType)+"\n");
